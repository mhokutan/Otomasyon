import requests, math, io, datetime, random
...
def _load_font(size=64):
    # GitHub runner'da bu font var; yoksa default
    try:
        return ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", size=size)
    except:
        return ImageFont.load_default()

def _sparkline(prices: List[float], w: int, h: int, color_up=(60, 220, 140), color_down=(240, 85, 85)) -> Image.Image:
    ...
    # son nokta
    d.ellipse([pts[-1][0]-8, pts[-1][1]-8, pts[-1][0]+8, pts[-1][1]+8], fill=col + (255,))
    return img

def build_crypto_items_and_images(ids: List[str], out_dir, stamp: str) -> (List[Dict], List[str]):
    # Veri
    ...
    for i, it in enumerate(items, 1):
        ...
        draw.text(((W-price_w)//2, 450), price_txt, fill=(255,255,255), font=f_big)
