...
def make_script_crypto(
    coins_data: Dict[str, Dict[str, float]], language: str = "en"
) -> Tuple[str, List[str]]:
    if not coins_data:
        if language.lower().startswith("tr"):
            return (
                "[ON SCREEN TEXT] Kripto özeti (veri alınamadı)\n[CTA] Takipte kal!",
                ["Kripto özeti (veri alınamadı)"],
            )
        return (
            "[ON SCREEN TEXT] 60-second crypto brief (no data)\n[CTA] Subscribe for daily briefs!",
    ...
    if language.lower().startswith("tr"):
        ...
        lines.append(
            "[HOOK] Bitcoin, Ethereum ve seçili altcoinlerde son 24 saatin özeti!"
        )
    else:
...
def make_script_news(
    headlines: List[Tuple[str, str]], language: str = "en"
) -> Tuple[str, List[str]]:
    if not headlines:
        if language.lower().startswith("tr"):
            return (
                "[ON SCREEN TEXT] 60 saniyelik özet (haber bulunamadı)\n[CTA] Takipte kal!",
                ["60 saniyelik özet (haber bulunamadı)"],
            )
        return (
            "[ON SCREEN TEXT] 60-second brief (no headlines)\n[CTA] Subscribe for daily briefs!",
    ...
    if language.lower().startswith("tr"):
        lines.append(
            "[CUT] 10 saniye kaldı; kısaca: Günün gelişmelerini tarayıp en net başlıkları seçiyoruz."
        )
        lines.append("[CTA] Her gün yeni 1 dakikalık özet için abone ol!")
    else:
        lines.append(
            "[CUT] 10 seconds left; quick recap: we track the day’s developments and pick the clearest headlines."
        )
...
def make_script_story(
    topic: Optional[str], language: str = "en"
) -> Tuple[str, List[str]]:
    ...
    **ignored: Any,
    ...
    main_part = (
        ...
        if captions
            ...
            if mode_l == "crypto"
            else ("A short story" if mode_l == "story" else "Top headlines")
        )
    )
    ...
    if mode_l == "crypto" and coins_data:
        ...
        for cid, d in coins_data.items():
            rows.append(
                f"{cid.upper()}: {_fmt_usd(d['usd'])} ({_fmt_pct(d['usd_24h_change'])})"
            )
        block = "\n".join(rows)
        ...
        if coin_rows:
            ...
            if not coin_list:
                coin_list = [
                    c.strip()
                    for c in (
                        _env("CRYPTO_COINS", "bitcoin,ethereum,solana") or ""
                    ).split(",")
                    if c.strip()
                ]
