...
FFMPEG_TIMEOUT = int(
    float(_env("FFMPEG_TIMEOUT", "240"))
)  # saniye (tek ffmpeg çağrısı)
...
def _picsum_urls(count: int) -> List[str]:
    ts = int(time.time())
    return [
        f"https://picsum.photos/{W}/{H}?random={ts+i+random.randint(0,99999)}"
...
def _unsplash_urls(count: int, keywords: List[str]) -> List[str]:
    ...
    for i in range(count):
        q = (
            ",".join(random.sample(keywords, k=min(2, len(keywords))))
            if keywords
            else "abstract"
        )
...
def _bg_urls_for_theme(
    theme: str, count: int, keywords=None, genre: str | None = None
) -> List[str]:
    kw_list: List[str] = []
...
# --------------- Metin yardımcıları ---------------
def _wrap_lines(
    drw: ImageDraw.ImageDraw, text: str, font: ImageFont.FreeTypeFont, max_w: int
) -> list[str]:
    words = (text or "").split()
    if not words:
        return [""]
...
def _load_presenter_avatar(size: int) -> Image.Image | None:
    ...
    try:
        ...
        if not p:
            ...
            dr.text(
                ((size - (bb[2] - bb[0])) // 2, (size - (bb[3] - bb[1])) // 2),
                txt,
                font=fnt,
...
def _compose_caption(
    bg: Image.Image, caption: str, theme: str, blink_variant: int = 0
) -> Image.Image:
    img = bg.convert("RGBA")
...
def _png_to_video(
    png: str, duration: float, out_mp4: str, fps: int = 60, zoom_per_sec: float = 0.0018
):
